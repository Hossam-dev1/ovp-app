...
export class EditComponent implements OnInit {
	...
	getDataByID() {
		this._crewTypeService.show(this.genre_ID).subscribe((resp: any) => {
			this.crewType_object = resp.body;
			this.patchUserData();
			console.log(this.crewType_object);
	...
	patchUserData() {
		...
		this.editForm.patchValue({
			key: this.crewType_object['key'],
			name: {
				en: this.crewType_object["name"]["en"],
				ar: this.crewType_object["name"]["ar"]
			}
		});
	}
	...
	submit() {
		...
		this._crewTypeService.edit(this.genre_ID, formData).subscribe((resp) => {
