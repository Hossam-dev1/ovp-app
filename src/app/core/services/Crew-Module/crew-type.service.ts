...
export class CrewTypeService {
	...
	add(data): Observable<any> {
		return this.HttpClient.post(environment.url() + 'admins/crew_types', data)
	}

	show(id: number) {
		return this.HttpClient.get(environment.url() + `admins/crew_types/${id}`, this.requestOptions)
	}

	edit(id: number, data): Observable<any> {
		return this.HttpClient.put(environment.url() + `admins/crew_types/${id}`, data)
	}

	delete(id: number) {
		return this.HttpClient.delete(environment.url() + `admins/crew_types/${id}`)
	}
}
