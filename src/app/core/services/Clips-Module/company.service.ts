...
export class CompanyService {
	...
	add(data): Observable<any> {
		return this.HttpClient.post(environment.url() + 'admins/companies', data)
	}

	show(id: number) {
		return this.HttpClient.get(environment.url() + `admins/companies/${id}`, this.requestOptions)
	}

	edit(id: number, data): Observable<any> {
		return this.HttpClient.put(environment.url() + `admins/companies/${id}`, data)
	}

	delete(id: number) {
		return this.HttpClient.delete(environment.url() + `admins/companies/${id}`)
	}
